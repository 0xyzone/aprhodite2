<x-app-layout>
    ...
    <div class="p-6 overflow-hidden bg-white rounded-md shadow-md dark:bg-dark-eval-1">
        <form action="{{ route('role.store') }}" class="max-w-sm" method="post">
            @csrf
            <div class="space-y-2">
                ...
                <x-input-with-icon-wrapper>
                    <x-slot name="icon">
                        <x-eos-role-binding aria-hidden="true" class="w-5 h-5" />
                    </x-slot>
                    <x-input withicon id="name" class="block w-full" type="text" name="name"
                        :value="old('name')" placeholder="{{ __('Type role name here...') }}" autofocus="true" />
                </x-input-with-icon-wrapper>
